...
CONTAINS
   ...
   use iarray,only : jmeanGLOBAL, absorb, rhokap
   ...
   open(newunit=u,file=trim(fileplace)//'jmean/absorb.dat',access='direct',status='REPLACE',form='unformatted',&
   recl=i)
   write(u,rec=1) absorb
   close(u)
